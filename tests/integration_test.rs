...
async fn find_by_time_range_returns_correct_records() {
    ...
    assert_eq!(results.len(), 4, "expected 4 records in [+2h, +5h], got {}", results.len());
}
...
async fn find_by_time_respects_limit() {
    ...
    let results = db.find_by_time(base - Duration::seconds(1), base + Duration::hours(1), 5).await.unwrap();
...
async fn semantic_search_finds_nearest_neighbors() {
    ...
    let results = db.find_similar(&[0.99, 0.01, 0.0, 0.0], 1, 0.8).await.unwrap();
...
async fn semantic_search_with_threshold_filters_results() {
    ...
    let hits = db.find_similar(&[1.0, 0.0, 0.0, 0.0], 5, 0.99).await.unwrap();
    assert_eq!(hits.len(), 1);

    // Orthogonal should fail
    let misses = db.find_similar(&[0.0, 1.0, 0.0, 0.0], 5, 0.5).await.unwrap();
...
async fn causal_descendants_returns_full_chain() {
    ...
    let a = RecordBuilder::new().schema("e.v1").data(serde_json::json!({"n":"A"})).timestamp(1).build().unwrap();
    let a_id = db.insert(a).await.unwrap();

    let b = RecordBuilder::new().schema("e.v1").data(serde_json::json!({"n":"B"})).timestamp(2)
        .caused_by(vec![a_id]).build().unwrap();
    let b_id = db.insert(b).await.unwrap();

    let c = RecordBuilder::new().schema("e.v1").data(serde_json::json!({"n":"C"})).timestamp(3)
        .caused_by(vec![b_id]).build().unwrap();
...
async fn causal_ancestors_returns_predecessors() {
    ...
    let root = RecordBuilder::new().schema("e.v1").data(serde_json::json!({"n":"root"})).timestamp(1).build().unwrap();
    let root_id = db.insert(root).await.unwrap();

    let mid = RecordBuilder::new().schema("e.v1").data(serde_json::json!({"n":"mid"})).timestamp(2)
        .caused_by(vec![root_id]).build().unwrap();
    let mid_id = db.insert(mid).await.unwrap();

    let leaf = RecordBuilder::new().schema("e.v1").data(serde_json::json!({"n":"leaf"})).timestamp(3)
        .caused_by(vec![mid_id]).build().unwrap();
...
async fn causal_depth_is_respected() {
    ...
    let a = RecordBuilder::new().schema("e.v1").data(serde_json::json!({"n":1})).timestamp(1).build().unwrap();
    let a_id = db.insert(a).await.unwrap();
    let b = RecordBuilder::new().schema("e.v1").data(serde_json::json!({"n":2})).timestamp(2).caused_by(vec![a_id]).build().unwrap();
    let b_id = db.insert(b).await.unwrap();
    let c = RecordBuilder::new().schema("e.v1").data(serde_json::json!({"n":3})).timestamp(3).caused_by(vec![b_id]).build().unwrap();
    let c_id = db.insert(c).await.unwrap();
    let d = RecordBuilder::new().schema("e.v1").data(serde_json::json!({"n":4})).timestamp(4).caused_by(vec![c_id]).build().unwrap();
...
async fn aqsl_schema_filter() {
    ...
    let results = db.query("FIND records WHERE schema = \"payment.v1\"").await.unwrap();
    assert_eq!(results.len(), 5);
    assert!(results.iter().all(|r| r.schema == "payment.v1"));
}

#[tokio::test]
async fn aqsl_limit_is_applied() {
    let db = fresh_db().await;
    for i in 0..20i64 {
        let r = RecordBuilder::new()
            .schema("e.v1")
    ...
    let results = db.query("FIND records WHERE schema = \"e.v1\" LIMIT 7").await.unwrap();
    assert_eq!(results.len(), 7);
}

#[tokio::test]
async fn aqsl_tag_filter() {
    let db = fresh_db().await;
    ...
    let results = db.query("FIND records WHERE tag env = \"production\"").await.unwrap();
    assert_eq!(results.len(), 1);
